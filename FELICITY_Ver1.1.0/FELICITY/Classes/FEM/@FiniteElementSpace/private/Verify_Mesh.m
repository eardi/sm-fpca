...
for ci = 1:obj.RefElem.Num_Comp
    Num_Fixed_Domain = length(obj.Fixed(ci).Domain);
    for di = 1:Num_Fixed_Domain
        Fixed_SubName = obj.Fixed(ci).Domain{di};
        Sub_Index = Mesh.Get_Subdomain_Index(Fixed_SubName);
        if (Sub_Index==0)
            disp(['This fixed sub-domain: ', Fixed_SubName]);
            disp(['    was not found in the given Mesh.']);
            disp('');
            error('Make sure your Mesh and/or your FiniteElementSpace is properly defined!');
        end
    end
end

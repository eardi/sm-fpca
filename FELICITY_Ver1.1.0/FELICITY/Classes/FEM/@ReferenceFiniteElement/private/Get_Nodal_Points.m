...
% error checking
ERR = max(abs((sum(obj.Nodal_BC_Coord,2) - 1)));
if ERR > 1e-15
    error('Barycentric coordinates should sum to 1.');
end
if size(obj.Simplex_Vtx,1)~=size(obj.Nodal_BC_Coord,2)
...
for ind = 1:obj.Num_Basis
    Nodal_Points(ind,:) = obj.Nodal_BC_Coord(ind,:) * obj.Simplex_Vtx; % row vector times multiple column vectors
end

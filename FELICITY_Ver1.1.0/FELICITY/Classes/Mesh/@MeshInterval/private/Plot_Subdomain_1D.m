...
VEC = obj.X(obj.Triangulation(Edge_Ind,2),:) - obj.X(obj.Triangulation(Edge_Ind,1),:);
scale = 0;
GD = obj.Geo_Dim;
hold on;
if (GD==3)
    Plot_Handle = quiver3(obj.X(obj.Triangulation(Edge_Ind,1),1),obj.X(obj.Triangulation(Edge_Ind,1),2),...
                          obj.X(obj.Triangulation(Edge_Ind,1),3),...
                          VEC(:,1),VEC(:,2),VEC(:,3),scale,'Color',Color_RGB,...
                          'LineWidth',LineWidth);
elseif (GD==2)
    Plot_Handle = quiver(obj.X(obj.Triangulation(Edge_Ind,1),1),obj.X(obj.Triangulation(Edge_Ind,1),2),...
                         VEC(:,1),VEC(:,2),scale,'Color',Color_RGB,'LineWidth',LineWidth);
else
    Plot_Handle = quiver(obj.X(obj.Triangulation(Edge_Ind,1),1),0*obj.X(obj.Triangulation(Edge_Ind,1),1),...

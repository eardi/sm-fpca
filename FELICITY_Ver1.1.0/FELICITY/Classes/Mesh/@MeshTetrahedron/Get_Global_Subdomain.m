...
%                into obj.X.  This is just like the 'SubData' input of
%                'Append_Subdomain'.
%   For example:
...
%       2-D Subdomain - output is a Fx3 array of (global) vertex indices, where
%                       each row represents a single triangle (face).
%       3-D Subdomain - output is a Tx4 array of (global) vertex indices, where
%                       each row represents a single tetrahedron (cell).
...
elseif (obj.Subdomain(Sub_Index).Dim==2)
    Global_Sub = obj.Get_Subdomain_Face_List(obj.Subdomain(Sub_Index));
elseif (obj.Subdomain(Sub_Index).Dim==3)
    Global_Sub = obj.Get_Subdomain_Tet_List(obj.Subdomain(Sub_Index));
else

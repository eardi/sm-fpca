...
%
%   Oriented_Tets = Mx4 matrix of oriented tetra data (indexes into obj.X).
...
if (size(Oriented_Tets,2)==4) % if we are given actual tets
    ...
    [TF1, LOC1] = ismember(Oriented_Tets(:,[1 2 3 4]),obj.Triangulation,'rows');
    [TF2, LOC2] = ismember(Oriented_Tets(:,[3 1 2 4]),obj.Triangulation,'rows');
    [TF3, LOC3] = ismember(Oriented_Tets(:,[2 3 1 4]),obj.Triangulation,'rows');
    
    [TF4, LOC4] = ismember(Oriented_Tets(:,[1 4 2 3]),obj.Triangulation,'rows');
    [TF5, LOC5] = ismember(Oriented_Tets(:,[2 1 4 3]),obj.Triangulation,'rows');
    [TF6, LOC6] = ismember(Oriented_Tets(:,[4 2 1 3]),obj.Triangulation,'rows');
    
    [TF7, LOC7] = ismember(Oriented_Tets(:,[1 3 4 2]),obj.Triangulation,'rows');
    [TF8, LOC8] = ismember(Oriented_Tets(:,[4 1 3 2]),obj.Triangulation,'rows');
    [TF9, LOC9] = ismember(Oriented_Tets(:,[3 4 1 2]),obj.Triangulation,'rows');
    
    [TF10, LOC10] = ismember(Oriented_Tets(:,[4 3 2 1]),obj.Triangulation,'rows');
    [TF11, LOC11] = ismember(Oriented_Tets(:,[2 4 3 1]),obj.Triangulation,'rows');
    [TF12, LOC12] = ismember(Oriented_Tets(:,[3 2 4 1]),obj.Triangulation,'rows');

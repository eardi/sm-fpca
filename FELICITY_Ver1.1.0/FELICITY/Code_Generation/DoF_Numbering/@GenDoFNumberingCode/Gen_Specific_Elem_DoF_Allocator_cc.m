...
if strcmp(obj.Elem(Elem_Num).Type,'CG')
    if and(obj.Elem(Elem_Num).Dim==1,strcmp(obj.Elem(Elem_Num).Domain,'interval'))
        CPP_Data_Type_str = Gen_Specific_Elem_DoF_Allocator_On_1D_Interval(obj,Elem_Num);
    elseif and(obj.Elem(Elem_Num).Dim==2,strcmp(obj.Elem(Elem_Num).Domain,'triangle'))
        CPP_Data_Type_str = Gen_Specific_Elem_DoF_Allocator_On_2D_Triangle(obj,Elem_Num);
    elseif and(obj.Elem(Elem_Num).Dim==3,strcmp(obj.Elem(Elem_Num).Domain,'tetrahedron'))
        CPP_Data_Type_str = Gen_Specific_Elem_DoF_Allocator_On_3D_Tetrahedron(obj,Elem_Num);
    else
        error('Not implemented!');
    end
elseif strcmp(obj.Elem(Elem_Num).Type,'DG')

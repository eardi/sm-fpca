function avd = Elem_DoF_Allocator_assign_vtx_dof_2D_triangle(obj,Elem)
%Elem_DoF_Allocator_assign_vtx_dof_2D_triangle
%
...
avd = avd.Append_CR('int EDA::Assign_Vtx_DoF(TRIANGLE_EDGE_SEARCH*  Tri_Edge_Search,   // inputs (list of triangles)');
avd = avd.Append_CR('                        const int              DoF_Offset)');
avd = avd.Append_CR('{');
avd = avd.Append_CR('    // for every vertex, store the first triangle (with local vertex index) that allocated DoFs for it');
avd = avd.Append_CR('    vector<int> Vtx_to_Tri[2];');
avd = avd.Append_CR('    Vtx_to_Tri[0].resize((unsigned int)(Tri_Edge_Search->Max_Vtx_Index+1),-1); // init to NULL value');
avd = avd.Append_CR('    Vtx_to_Tri[1].resize((unsigned int)(Tri_Edge_Search->Max_Vtx_Index+1),-1); // init to NULL value');
avd = avd.Append_CR('');
...
avd = avd.Append_CR('        for (int ti=0; ti < Tri_Edge_Search->Num_Tri; ti++)');
avd = avd.Append_CR('            {');
avd = avd.Append_CR('            // loop thru all vertices of each triangle');
avd = avd.Append_CR('            for (int vtxi=0; vtxi < NUM_VTX; vtxi++)');
avd = avd.Append_CR('                {');
avd = avd.Append_CR('                const unsigned int Vertex = (unsigned int) Tri_Edge_Search->Tri_DoF[vtxi][ti];');
avd = avd.Append_CR('                const int tri_ind         =     ti;');
...
avd = avd.Append_CR('                if (Vtx_to_Tri[0][Vertex]==-1)');
avd = avd.Append_CR('                    {');
avd = avd.Append_CR('                    VTX_COUNT++;');
avd = avd.Append_CR('');
avd = avd.Append_CR('                    // remember which triangle is associated with this vertex');
avd = avd.Append_CR('                    Vtx_to_Tri[0][Vertex] = tri_ind;');
avd = avd.Append_CR('                    // remember the local vertex');
avd = avd.Append_CR('                    Vtx_to_Tri[1][Vertex] = vtx_ind;');
avd = avd.Append_CR('');
...
avd = avd.Append_CR('                    {');
avd = avd.Append_CR('                    const int old_ti      = Vtx_to_Tri[0][Vertex];');
avd = avd.Append_CR('                    const int old_vtx_ind = Vtx_to_Tri[1][Vertex];');
avd = avd.Append_CR('');
...
avd = avd.Append_CR('        Tri_Edge_Search->Num_Unique_Vertices = VTX_COUNT;');
avd = avd.Append_CR('        }');

...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    Node.V[2][1][1] = 5; // Set2, V1
    Node.V[2][2][1] = 6; // Set2, V2

    Node.E[1][1][1] = 3; // Set1, E1
    Node.E[1][1][2] = 4; // Set1, E1
        ...
        for (int ei=0; ei < Edge_Point_Search->Num_Edge; ei++)
            ...
            for (int vtxi=0; vtxi < NUM_VTX; vtxi++)
                ...
                if (Vtx_to_Edge[0][Vertex]==-1)
                    ...
                    // Set 2: add more DoFs
                    Vtx_DoF_Counter++;
                    cell_dof[ Node.V[2][vtx_ind][1] ][ei] = Vtx_DoF_Counter;
                    /*------------   END: Auto Generate ------------*/
                    }
                else
                    {
                    ...
                    cell_dof[ Node.V[2][vtx_ind][1] ][ei] = cell_dof[ Node.V[2][old_vtx_ind][1] ][old_ei];
                    /*------------   END: Auto Generate ------------*/
                    }
                }
            }
                         ...
                         const int           DoF_Offset)
{
    ...
    if (Num_DoF_Per_Edge > 0)
        ...
        for (int ei=0; ei < Edge_Point_Search->Num_Edge; ei++)
            ...
            // Set 1: add more DoFs
            Edge_DoF_Counter++;
            cell_dof[ Node.E[1][edge_ind][1] ][ei] = Edge_DoF_Counter;
            Edge_DoF_Counter++;
            cell_dof[ Node.E[1][edge_ind][2] ][ei] = Edge_DoF_Counter;
            /*------------   END: Auto Generate ------------*/
            }
        }

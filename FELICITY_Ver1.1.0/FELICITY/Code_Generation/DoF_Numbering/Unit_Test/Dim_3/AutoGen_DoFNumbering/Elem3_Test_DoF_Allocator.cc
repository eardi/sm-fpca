...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    Node.F[1][1][1] = 1; // Set1, F1
    Node.F[1][1][2] = 2; // Set1, F1
    Node.F[1][1][3] = 3; // Set1, F1
    Node.F[1][1][4] = 4; // Set1, F1
    Node.F[1][1][5] = 5; // Set1, F1
    Node.F[1][1][6] = 6; // Set1, F1
    Node.F[1][2][1] = 7; // Set1, F2
    Node.F[1][2][2] = 8; // Set1, F2
    Node.F[1][2][3] = 9; // Set1, F2
    Node.F[1][2][4] = 10; // Set1, F2
    Node.F[1][2][5] = 11; // Set1, F2
    Node.F[1][2][6] = 12; // Set1, F2
    Node.F[1][3][1] = 13; // Set1, F3
    Node.F[1][3][2] = 14; // Set1, F3
    Node.F[1][3][3] = 15; // Set1, F3
    Node.F[1][3][4] = 16; // Set1, F3
    Node.F[1][3][5] = 17; // Set1, F3
    Node.F[1][3][6] = 18; // Set1, F3
    Node.F[1][4][1] = 19; // Set1, F4
    Node.F[1][4][2] = 20; // Set1, F4
    Node.F[1][4][3] = 21; // Set1, F4
    Node.F[1][4][4] = 22; // Set1, F4
    Node.F[1][4][5] = 23; // Set1, F4
    Node.F[1][4][6] = 24; // Set1, F4

    Node.T[1][1][1] = 25; // Set1, T1
    ...
    if (Num_DoF_Per_Face > 0)
        ...
        for (fi=Tet_Data->Face_List.begin(); fi!=Tet_Data->Face_List.end(); ++fi)
            ...
            if (NEW_FACE)
                ...
                // Set 1: add more DoFs
                Face_DoF_Counter++;
                SF_DoF[1][1] = Face_DoF_Counter;
                Face_DoF_Counter++;
                SF_DoF[1][2] = Face_DoF_Counter;
                Face_DoF_Counter++;
                SF_DoF[1][3] = Face_DoF_Counter;
                Face_DoF_Counter++;
                SF_DoF[1][4] = Face_DoF_Counter;
                Face_DoF_Counter++;
                SF_DoF[1][5] = Face_DoF_Counter;
                Face_DoF_Counter++;
                SF_DoF[1][6] = Face_DoF_Counter;
                /*------------   END: Auto Generate ------------*/
                }
            else
                {
            ...
            if (NEW_FACE)
                ...
                cell_dof[ Node.F[1][local_face_index][1] ][cell_index] = SF_DoF[1][1];
                cell_dof[ Node.F[1][local_face_index][2] ][cell_index] = SF_DoF[1][2];
                cell_dof[ Node.F[1][local_face_index][3] ][cell_index] = SF_DoF[1][3];
                cell_dof[ Node.F[1][local_face_index][4] ][cell_index] = SF_DoF[1][4];
                cell_dof[ Node.F[1][local_face_index][5] ][cell_index] = SF_DoF[1][5];
                cell_dof[ Node.F[1][local_face_index][6] ][cell_index] = SF_DoF[1][6];
                /*------------   END: Auto Generate ------------*/
                }
            // allocate DoFs on opposite (second) face in *reverse* order (with respect to each edge) and swap two of the edges
                ...
                if (NOT_SWAP==1) // swap edges 2 & 3
                    ...
                    // Set 1: write DoFs in reverse order from before
                    // edge 1
                    cell_dof[ Node.F[1][local_face_index][6] ][cell_index] = SF_DoF[1][1];
                    cell_dof[ Node.F[1][local_face_index][5] ][cell_index] = SF_DoF[1][2];
                    // edge 2
                    cell_dof[ Node.F[1][local_face_index][4] ][cell_index] = SF_DoF[1][3];
                    cell_dof[ Node.F[1][local_face_index][3] ][cell_index] = SF_DoF[1][4];
                    // edge 3
                    cell_dof[ Node.F[1][local_face_index][2] ][cell_index] = SF_DoF[1][5];
                    cell_dof[ Node.F[1][local_face_index][1] ][cell_index] = SF_DoF[1][6];
                    /*------------   END: Auto Generate ------------*/
                    }
                    ...
                    // Set 1: write DoFs in reverse order from before
                    // edge 1
                    cell_dof[ Node.F[1][local_face_index][4] ][cell_index] = SF_DoF[1][1];
                    cell_dof[ Node.F[1][local_face_index][3] ][cell_index] = SF_DoF[1][2];
                    // edge 2
                    cell_dof[ Node.F[1][local_face_index][2] ][cell_index] = SF_DoF[1][3];
                    cell_dof[ Node.F[1][local_face_index][1] ][cell_index] = SF_DoF[1][4];
                    // edge 3
                    cell_dof[ Node.F[1][local_face_index][6] ][cell_index] = SF_DoF[1][5];
                    cell_dof[ Node.F[1][local_face_index][5] ][cell_index] = SF_DoF[1][6];
                    /*------------   END: Auto Generate ------------*/
                    }
                }
            }
                        ...
                        const int               DoF_Offset)
{
    ...
    if (Num_DoF_Per_Tet > 0)
        ...
        for (int ti=0; ti < Tet_Data->Num_Tet; ti++)
            ...
            // Set 1: add more DoFs
            Tet_DoF_Counter++;
            cell_dof[ Node.T[1][tet_ind][1] ][ti] = Tet_DoF_Counter;
            /*------------   END: Auto Generate ------------*/
            }
        }

...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    Node.F[1][2][1] = 4; // Set1, F2
    Node.F[1][2][2] = 5; // Set1, F2
    Node.F[1][2][3] = 6; // Set1, F2
    Node.F[1][3][1] = 7; // Set1, F3
    Node.F[1][3][2] = 8; // Set1, F3
    Node.F[1][3][3] = 9; // Set1, F3
    Node.F[1][4][1] = 10; // Set1, F4
    Node.F[1][4][2] = 11; // Set1, F4
    Node.F[1][4][3] = 12; // Set1, F4

    Node.F[2][1][1] = 13; // Set2, F1
    Node.F[2][2][1] = 14; // Set2, F2
    Node.F[2][3][1] = 15; // Set2, F3
    Node.F[2][4][1] = 16; // Set2, F4
    ...
    if (Num_DoF_Per_Face > 0)
        ...
        for (fi=Tet_Data->Face_List.begin(); fi!=Tet_Data->Face_List.end(); ++fi)
            ...
            if (NEW_FACE)
                ...
                // Set 2: write DoFs in an arbitrary, but fixed order
                cell_dof[ Node.F[2][local_face_index][1] ][cell_index] = SF_DoF[2][1];
                /*------------   END: Auto Generate ------------*/
                }
            // allocate DoFs on opposite (second) face in *reverse* order (with respect to each edge) and swap two of the edges
                ...
                if (NOT_SWAP==1) // swap edges 2 & 3
                    ...
                    // Set 2: write DoF for center of face
                    cell_dof[ Node.F[2][local_face_index][1] ][cell_index] = SF_DoF[2][1];
                    /*------------   END: Auto Generate ------------*/
                    }
                    ...
                    // Set 1: write DoFs in reverse order from before
                    // edge 1
                    cell_dof[ Node.F[1][local_face_index][3] ][cell_index] = SF_DoF[1][1];
                    // edge 2
                    cell_dof[ Node.F[1][local_face_index][2] ][cell_index] = SF_DoF[1][2];
                    // edge 3
                    cell_dof[ Node.F[1][local_face_index][1] ][cell_index] = SF_DoF[1][3];
                    // Set 2: write DoF for center of face
                    cell_dof[ Node.F[2][local_face_index][1] ][cell_index] = SF_DoF[2][1];
                    /*------------   END: Auto Generate ------------*/
                    }

...
typedef struct
{
    ...
    // edge_info[2]:   index of the tetrahedron that contains the edge
...
typedef struct
{
    ...
    //                   note: these index into the global vertex list
    // face_info[3]:     index of the tetrahedron that contains the face
    // face_info[4]:     local face index in the enclosing cell;  value: 1,2,3,4
	// face_info[5]:     local index (0,1, or 2) that the zeroth vertex (i.e. face_info[0])
	//                   had *before* face_info[0,1,2] was put into ascending order.
}
FACE_TO_CELL;
...
/* this ensures that EI.edge_info[0] < EI.edge_info[1] and adjusts EI.edge_info[3]
   ...
   orientation of the edge in the triangulation. */
...
/* this ensures that FI.face_info[0] < FI.face_info[1] < FI.face_info[2] and sets
   FI.face_info[5] = the value of FI.face_info[0] *before* this function runs. */
void TD::Ensure_Ordered_Face(FACE_TO_CELL& FI)     // input to be modified
{
    ...
    if (V0 > V1)
		...
		if (V1 > V2)
			...
			// record original position of FI.face_info[0]
			FI.face_info[5] = 2;
			}
		else if (V1 < V2)
			{
		...
		if (V1 > V2)
			...
			{
			// no change needed! already ordered.
			// record original position of FI.face_info[0]
			FI.face_info[5] = 0;
			}
		else
...
/***************************************************************************************/
/* determine which edge (1, 2, or 3) *not* to swap in face, given the original positions
   of the first ordered face vertex, i.e. FI.face_info[0], in the the first face and
   second face.
   Example: this means that if Local Edge 1 is *not* to be swapped,
            then Local Edges 2 and 3 are to be swapped.   */
unsigned int TD::Determine_Edge_NOT_To_Swap_In_Face(const unsigned int& first_ind,  // inputs
                                                    const unsigned int& second_ind) // inputs
{
	static const unsigned int MAT[3][3] = { {1, 3, 2},
	                                        {3, 2, 1},
											{2, 1, 3} };
	const unsigned int INDEX = MAT[first_ind][second_ind];
	return INDEX;
}
/***************************************************************************************/

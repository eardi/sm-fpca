...
% allocate DoFs
BDM1_DoFmap = feval(str2func(mexName_DoF),uint32(Mesh.Triangulation));
...
v = Get_BDM1_Interpolant_of_Function(Mesh.X,Mesh.Triangulation,BDM1_DoFmap,Omega_Orient,F1);
...
BC = Mesh.refToBary(Omega_Interp_Data{2});
Omega_Interp_Pts = Mesh.baryToCart(Cell_Omega_Indices, BC);
...
BC = Mesh_Gamma.refToBary(Gamma_Interp_Data{2});
Gamma_Interp_Pts = Mesh_Gamma.baryToCart(Cell_Gamma_Indices, BC);

% interpolate!
tic
INTERP = feval(str2func(mexName_Interp),Mesh.X,uint32(Mesh.Triangulation),Omega_Orient,Omega_Mesh_Subdomains,...

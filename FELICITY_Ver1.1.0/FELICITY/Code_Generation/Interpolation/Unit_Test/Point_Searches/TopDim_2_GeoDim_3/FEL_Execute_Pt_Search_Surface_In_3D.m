...
z = 1 - ( (Mesh.X(:,1) - 1).^2 + (Mesh.X(:,2) - 1).^2 );
Mesh = Mesh.Set_X([Mesh.X(:,1:2), z]);
...
% search!
tic
SEARCH = feval(str2func(mexName),Mesh.X,uint32(Mesh.Triangulation),[],[],Gamma_Given_Points);
toc
...
Edges = Mesh.edges;
EV = Mesh.X(Edges(:,2),:) - Mesh.X(Edges(:,1),:);

function status = Gen_CPP_Quad_Loop_Code(fid_Open,SYMM,Ccode_Frag,v,u)
...
if and(YES_ROW, YES_COL)
    status = Row_Col_Quad_Loop(fid_Open,SYMM,Ccode_Frag,Det_Jac_str);
elseif and(YES_ROW, ~YES_COL)
    status = Row_ONLY_Quad_Loop(fid_Open,Ccode_Frag,Det_Jac_str);
elseif and(~YES_ROW, YES_COL)
    status = Col_ONLY_Quad_Loop(fid_Open,Ccode_Frag,Det_Jac_str);
else
    % this is some kind of norm error quadrature
    status = JUST_Quad_Loop(fid_Open,Ccode_Frag,Det_Jac_str);
end

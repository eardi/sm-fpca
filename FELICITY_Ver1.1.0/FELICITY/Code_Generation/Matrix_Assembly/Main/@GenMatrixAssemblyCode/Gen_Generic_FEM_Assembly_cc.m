...
% output text-lines
fprintf(fid, ['    ', obj.BEGIN_Auto_Gen, ENDL]);
fprintf(fid, ['    ', '// setup access to FEM basis functions', ENDL]);
for int_index = 1:length(FS.Integration)
    ...
    for bi = 1:length(BF_Names)
        SN = BF_Set(BF_Names{bi}).Space_Name;
        CPP_SPACE = FS.Get_CPPdefine_Space_Name(SN);
        fprintf(fid, ['    ', BF_Set(BF_Names{bi}).CPP_Var,...
                      '.Setup_Function_Space(prhs[', CPP_SPACE.MEX_DoFmap, ']);', ENDL]);
        fprintf(fid, ['    ', BF_Set(BF_Names{bi}).CPP_Var,...
                      '.Mesh = &', BF_Set(BF_Names{bi}).GeomFunc.CPP.Identifier_Name, ';', ENDL]);
    end
end
fprintf(fid, ['    ', obj.END_Auto_Gen, ENDL]);
fprintf(fid, ['', ENDL]);
...
% output text-lines
fprintf(fid, ['    ', obj.BEGIN_Auto_Gen, ENDL]);
fprintf(fid, ['    ', '// setup access to external FEM functions', ENDL]);
...
for int_index = 1:length(FS.Integration)
    ...
    for index = 1:length(CF_Names)
        ...
        % output text-lines
        fprintf(fid, ['    ', CF_Set(CF_Names{index}).CPP_Var,...
                      '.Setup_Function_Space(prhs[', CPP_FUNC.MEX_Node_Value, '], &',...
                       BF_Set(CF_Set(CF_Names{index}).Space_Name).CPP_Var,  ');', ENDL]);
    end
end
fprintf(fid, ['    ', obj.END_Auto_Gen, ENDL]);
fprintf(fid, ['', ENDL]);
...
%     Fobj[0] = new FEM_Matrix_Specific(this, 0, &Scalar_P2_phi, &Scalar_P2_phi);
%     /*------------   END: Auto Generate ------------*/

%%%%%%%
% output text-lines
fprintf(fid, ['    ', obj.BEGIN_Auto_Gen, ENDL]);
fprintf(fid, ['    ', '// setup the desired matrices to assemble', ENDL]);
for index = 1:NUM_MAT
    MAT_CPP = FM.Get_Matrix_CPP_Info(Matrix_Names{index});
    ...
    FEM_Matrix_ARG_str = [MAT_CPP.Var_Name, ' = new ', MAT_CPP.Data_Type_Name,...
                         '(&', MAT_CPP.Sparse_Data_Name, ', ', '&', Row_CPP_Var,...
                         ', &', Col_CPP_Var, ');'];
...
for index = 1:NUM_MAT
    ...
    for gi = 1:Num_GeomFunc
        ...
        fprintf(fid, ['    ', MAT_CPP.Var_Name, '->', Geom_CPP.Identifier_Name, ' = &', Geom_CPP.Identifier_Name, ';', ENDL]);
    end
    for bi = 1:length(Basis_Func)
        BF_Var_Name = Basis_Func{bi}.CPP_Var;
        fprintf(fid, ['    ', MAT_CPP.Var_Name, '->', BF_Var_Name, ' = &', BF_Var_Name, ';', ENDL]);
    end
    for ci = 1:length(Coef_Func)
        CF_Var_Name = Coef_Func{ci}.CPP_Var;
        fprintf(fid, ['    ', MAT_CPP.Var_Name, '->', CF_Var_Name, ' = &', CF_Var_Name, ';', ENDL]);
    end

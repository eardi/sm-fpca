...
% Copyright (c) 03-05-2013,  Shawn W. Walker
...
if (Global_Hold_All_Top_Dim==1)
    ...
    if (Subdomain_Top_Dim==1)
        Sub_Sym_Map = sym('u_hat'); % identity map
        if (DoI_Top_Dim==1) % codim = 0
            DoI_Sym_Map = sym('x_hat'); % identity map
        else
            error('Invalid!');
        end
    else
...
elseif (Global_Hold_All_Top_Dim==2)
    if (Subdomain_Top_Dim==2) % codim = 0
        Sub_Sym_Map = sym('[u_hat;v_hat]'); % identity map
        if (DoI_Top_Dim==2)
            DoI_Sym_Map = sym('[x_hat;y_hat]'); % identity map
        elseif (DoI_Top_Dim==1) % codim = 1
            DoI_Sym_Map = map_unit_interval_to_tri_edge(sym('x_hat'),entity_ind(1));
        else
            error('Invalid!');
        end
    elseif (Subdomain_Top_Dim==1)
        Sub_Sym_Map = map_unit_interval_to_tri_edge(sym('u_hat'),entity_ind(1));
...
elseif (Global_Hold_All_Top_Dim==3)
    if (Subdomain_Top_Dim==3)
        Sub_Sym_Map = sym('[u_hat; v_hat; w_hat]'); % identity map
        if (DoI_Top_Dim==3) % codim = 0
            DoI_Sym_Map = sym('[x_hat; y_hat; z_hat]'); % identity map
        elseif (DoI_Top_Dim==2) % codim = 1
            DoI_Sym_Map = map_unit_triangle_to_tet_face(sym('[x_hat;y_hat]'),entity_ind(1));
        elseif (DoI_Top_Dim==1) % codim = 2
            DoI_Sym_Map = map_unit_interval_to_tet_edge(sym('x_hat'),entity_ind(1));
        else
            error('Invalid!');
        end
        
    elseif (Subdomain_Top_Dim==2)
        Sub_Sym_Map = map_unit_triangle_to_tet_face(sym('[u_hat; v_hat]'),entity_ind(1));
        if (DoI_Top_Dim==2) % codim = 1
            DoI_Sym_Map = sym('[x_hat; y_hat]'); % identity map
        elseif (DoI_Top_Dim==1) % codim = 2
            % if this is the special case, then we want the second entity index
            DoI_Sym_Map = map_unit_interval_to_tri_edge(sym('x_hat'),entity_ind(end));
        else
            error('Invalid!');
        end
    elseif (Subdomain_Top_Dim==1)
        Sub_Sym_Map = map_unit_interval_to_tet_edge(sym('u_hat'),entity_ind(1));

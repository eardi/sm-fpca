...
coefs = sym('coef_%d%d',[Num_Basis, obj.GeoDim]);
local_basis = sym('Geo_phi_%d_d%d',[Num_Basis, obj.TopDim]);
...
for ir=1:obj.GeoDim
    for ic=1:obj.TopDim
        ...
        for ib=1:Num_Basis
            Geo_phi_S_str = ['Geo_phi_', num2str(ib), '_d', num2str(ic)];
            Num_Deriv = [0 0 0];
            Num_Deriv(ic) = 1;
            ...
            coef_S_str = ['coef_', num2str(ib), num2str(ir)];

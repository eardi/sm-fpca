function CODE = COEF_FUNC_d2_ds2_C_Code(obj)
%COEF_FUNC_d2_ds2_C_Code
%
%   Generate C-code for computing coefficient functions.
%     d^2/ds^2 f_i(qp) = SUM^{NUM_BASIS}_{k=1} c_{ik} * d^2/ds^2 phi_k(qp), where
...
if (obj.GeoMap.TopDim==1)
    ...
    BF  = ['basis_func->', f_d2_ds2_str, '[basis_i][qp_i]', '.a'];
    BF0 = ['basis_func->', f_d2_ds2_str, '[0][qp_i]', '.a'];
    CF = [f_d2_ds2_str, '[nc_i][qp_i]', '.a'];
    TAB = '    ';
    TAB2 = [TAB, TAB];
    ...
    Defn_Hdr = '// 2nd arc-length derivative of coefficient function';
    Loop_Hdr = '// get 2nd arc-length derivative of coefficient function';
    Loop_Comment = '// loop through all components (indexing is in the C style)';
    CONST_VAR = obj.Is_Quantity_Constant('d2_ds2');
    CODE = obj.create_coef_func_declaration_and_eval_code(f_d2_ds2_str,TYPE_str,EVAL_STR,...
                                Defn_Hdr,Loop_Hdr,Loop_Comment,CONST_VAR);
else
    CODE = []; % d^2/ds^2 is only useful in 1-D
end

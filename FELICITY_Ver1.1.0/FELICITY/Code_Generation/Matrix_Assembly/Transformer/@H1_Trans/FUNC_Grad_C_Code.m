...
elseif (obj.GeoMap.TopDim==2) % reference domain is 2-D
    if (obj.GeoMap.GeoDim==2)
        ...
        EVAL_STR(3).line = [TAB, 'const VEC_2x1  grad_basis_vec(', BF_D0, ', ', BF_D1, ');'];
        TEMP_STR = ['Mat_Transpose_Vec(', 'Mesh->', Inv_Grad_CPP_Name, QP_str, ', grad_basis_vec, ',...
    ...
    elseif (obj.GeoMap.GeoDim==3)
        ...
        EVAL_STR(3).line = [TAB, 'const VEC_2x1  grad_basis_vec(', BF_D0, ', ', BF_D1, ');'];
        EVAL_STR(4).line = [TAB, 'VEC_2x1 VL; // map local gradient to the tangent space'];
        TEMP_STR = ['Mat_Transpose_Vec(', 'Mesh->', Inv_Metric_CPP_Name, QP_str, ', grad_basis_vec, VL);'];
        EVAL_STR(5).line = [TAB, TEMP_STR];
...
elseif (obj.GeoMap.TopDim==3) % reference domain is 3-D
    if (obj.GeoMap.GeoDim==3)
        ...
        EVAL_STR(3).line = [TAB, 'const VEC_3x1  grad_basis_vec(', BF_D0, ', ', BF_D1, ', ', BF_D2, ');'];
        TEMP_STR = ['Mat_Transpose_Vec(', 'Mesh->', Inv_Grad_CPP_Name, QP_str, ', grad_basis_vec, ',...
                    f_Grad_str, '[basis_i][qp_i]', ');'];
        EVAL_STR(4).line = [TAB, TEMP_STR];
    else

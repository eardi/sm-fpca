...
% Copyright (c) 08-08-2014,  Shawn W. Walker
...
BF_DD0  = 'phi_0_Basis_Val_2_0_0[qp_i][basis_i]';
BF_D0D1 = 'phi_0_Basis_Val_1_1_0[qp_i][basis_i]';
BF_D0D2 = 'phi_0_Basis_Val_1_0_1[qp_i][basis_i]';
BF_DD1  = 'phi_0_Basis_Val_0_2_0[qp_i][basis_i]';
BF_D1D2 = 'phi_0_Basis_Val_0_1_1[qp_i][basis_i]';
BF_DD2  = 'phi_0_Basis_Val_0_0_2[qp_i][basis_i]';
...
if (obj.GeoMap.TopDim==1) % reference domain is 1-D
    if (obj.GeoMap.GeoDim==1)
        ...
        TEMP_STR = [BF_DD0, ' * ', 'Inv_Det_Jac_Square']; % first part of formula
...
elseif (obj.GeoMap.TopDim==2) % reference domain is 2-D
    if (obj.GeoMap.GeoDim==2)
        ...
        % evaluate!
        EVAL_STR( 3).line = [TAB, '// setup the hessian matrix of the local (scalar) basis function'];
        EVAL_STR( 4).line = [TAB, 'MAT_2x2  hess_basis_func_local;'];
        EVAL_STR( 5).line = [TAB, 'hess_basis_func_local.m[0][0] = ', BF_DD0,  ';'];
        EVAL_STR( 6).line = [TAB, 'hess_basis_func_local.m[0][1] = ', BF_D0D1, ';'];
        EVAL_STR( 7).line = [TAB, 'hess_basis_func_local.m[1][0] = ', BF_D0D1, ';'];
        EVAL_STR( 8).line = [TAB, 'hess_basis_func_local.m[1][1] = ', BF_DD1,  ';'];
        EVAL_STR( 9).line = [TAB, '// compute main part of formula'];
        EVAL_STR(10).line = [TAB, 'MAT_2x2  Temp_MAT;'];
        EVAL_STR(11).line = [TAB, 'Mat_Mat(hess_basis_func_local, ', Inv_Grad_Quad_str, ', Temp_MAT);'];
        EVAL_STR(12).line = [TAB, 'Mat_Transpose_Mat(', Inv_Grad_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
        if ~CONST_JAC % map is non-linear, then need the more general formula
            EVAL_STR(13).line = [TAB, '// map is non-linear, so compute other part of formula'];
            EVAL_STR(14).line = [TAB, '// scale hessian(PHI^{-1}) by d/dx of basis function, and add'];
            EVAL_STR(15).line = [Hess_Inv_Map_Quad_str, '.Extract_Comp_Matrix(0, Temp_MAT);'];
            EVAL_STR(16).line = ['Scalar_Mult_Matrix(Temp_MAT, ', BF_D0, ', Temp_MAT);'];
            EVAL_STR(17).line = ['Add_Matrix(', f_Hess_Basis_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
            
            EVAL_STR(18).line = [TAB, '// scale hessian(PHI^{-1}) by d/dy of basis function, and add'];
            EVAL_STR(19).line = [Hess_Inv_Map_Quad_str, '.Extract_Comp_Matrix(1, Temp_MAT);'];
            EVAL_STR(20).line = ['Scalar_Mult_Matrix(Temp_MAT, ', BF_D1, ', Temp_MAT);'];
            EVAL_STR(21).line = ['Add_Matrix(', f_Hess_Basis_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
        end
    elseif (obj.GeoMap.GeoDim==3) % surface in 3-D
        ...
        EVAL_STR( 4).line = [TAB, 'const VEC_2x1  grad_basis(', BF_D0, ', ', BF_D1, '); // put into convenient struct'];
        EVAL_STR( 5).line = [TAB, 'MAT_2x2  hess_basis; // put into convenient struct'];
        EVAL_STR( 6).line = [TAB, 'hess_basis.m[0][0] = ', BF_DD0, ';'];
        EVAL_STR( 7).line = [TAB, 'hess_basis.m[0][1] = ', BF_D0D1, ';'];
        EVAL_STR( 8).line = [TAB, 'hess_basis.m[1][0] = hess_basis.m[0][1];'];
        EVAL_STR( 9).line = [TAB, 'hess_basis.m[1][1] = ', BF_DD1, ';'];
        EVAL_STR(10).line = [TAB, 'MAT_3x2  sub_MAT, temp_hess_MAT; // sub-matrix and temporary'];
        EVAL_STR(11).line = [TAB, 'for (unsigned int kk = 0; (kk < 3); kk++)'];
        EVAL_STR(12).line = [TAB, 'for (unsigned int rr = 0; (rr < 2); rr++)'];
        EVAL_STR(13).line = [TAB2, '{'];
        EVAL_STR(14).line = [TAB2, '// compute entry of sub-matrix'];
        if ~CONST_JAC % map is non-linear, then need the more general formula
            EVAL_STR(15).line = [TAB2, 'double VAL_A = 0.0;'];
            EVAL_STR(16).line = [TAB2, 'double VAL_B = 0.0;'];
            EVAL_STR(17).line = [TAB2, 'for (unsigned int ii = 0; (ii < 2); ii++)'];
            EVAL_STR(18).line = [TAB2, 'for (unsigned int jj = 0; (jj < 2); jj++)'];
            EVAL_STR(19).line = [TAB3, '{'];
            EVAL_STR(20).line = [TAB3, 'VAL_A += ', Grad_Quad_str, '.m[kk][ii] * ', Inv_Metric_Quad_str, '.m[ii][jj] * ',...
                                       'hess_basis.m[rr][jj];'];
            EVAL_STR(21).line = [TAB3, 'VAL_B += ( (', Hess_Quad_str, '.m[kk][rr][ii] * ', Inv_Metric_Quad_str, '.m[ii][jj]) + (',...
                                       Grad_Quad_str, '.m[kk][ii] * ', Grad_Inv_Metric_Quad_str, '.m[rr][ii][jj]) )',...
                                       ' * grad_basis.v[jj]; // this term needed because map is non-linear'];
            EVAL_STR(22).line = [TAB3, '}'];
            EVAL_STR(23).line = [TAB2, 'sub_MAT.m[kk][rr] = VAL_A + VAL_B;'];
        else
            % use simpler formula
            EVAL_STR(15).line = [TAB2, 'double VAL_entry = 0.0;'];
            EVAL_STR(16).line = [TAB2, 'for (unsigned int ii = 0; (ii < 2); ii++)'];
            EVAL_STR(17).line = [TAB2, 'for (unsigned int jj = 0; (jj < 2); jj++)'];
            EVAL_STR(18).line = [TAB3, '{'];
            EVAL_STR(19).line = [TAB3, 'VAL_entry += ', Grad_Quad_str, '.m[kk][ii] * ', Inv_Metric_Quad_str, '.m[ii][jj] * ',...
                                       'hess_basis.m[rr][jj];'];
            EVAL_STR(20).line = [TAB3, '}'];
            EVAL_STR(21).line = [TAB2, 'sub_MAT.m[kk][rr] = VAL_entry;'];
        end
...
elseif (obj.GeoMap.TopDim==3) % reference domain is 3-D
    if (obj.GeoMap.GeoDim==3) % this is the only valid option!
        ...
        % evaluate!
        EVAL_STR( 3).line = [TAB, '// setup the hessian matrix of the local (scalar) basis function'];
        EVAL_STR( 4).line = [TAB, 'MAT_3x3  hess_basis_func_local;'];
        EVAL_STR( 5).line = [TAB, 'hess_basis_func_local.m[0][0] = ', BF_DD0,  ';'];
        EVAL_STR( 6).line = [TAB, 'hess_basis_func_local.m[0][1] = ', BF_D0D1, ';'];
        EVAL_STR( 7).line = [TAB, 'hess_basis_func_local.m[0][2] = ', BF_D0D2, ';'];
        EVAL_STR( 8).line = [TAB, 'hess_basis_func_local.m[1][0] = ', BF_D0D1, ';'];
        EVAL_STR( 9).line = [TAB, 'hess_basis_func_local.m[1][1] = ', BF_DD1,  ';'];
        EVAL_STR(10).line = [TAB, 'hess_basis_func_local.m[1][2] = ', BF_D1D2, ';'];
        EVAL_STR(11).line = [TAB, 'hess_basis_func_local.m[2][0] = ', BF_D0D2, ';'];
        EVAL_STR(12).line = [TAB, 'hess_basis_func_local.m[2][1] = ', BF_D1D2, ';'];
        EVAL_STR(13).line = [TAB, 'hess_basis_func_local.m[2][2] = ', BF_DD2,  ';'];
        EVAL_STR(14).line = [TAB, '// compute main part of formula'];
        EVAL_STR(15).line = [TAB, 'MAT_3x3  Temp_MAT;'];
        EVAL_STR(16).line = [TAB, 'Mat_Mat(hess_basis_func_local, ', Inv_Grad_Quad_str, ', Temp_MAT);'];
        EVAL_STR(17).line = [TAB, 'Mat_Transpose_Mat(', Inv_Grad_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
        if ~CONST_JAC % map is non-linear, then need the more general formula
            EVAL_STR(18).line = [TAB, '// map is non-linear, so compute other part of formula'];
            EVAL_STR(19).line = [TAB, '// scale hessian(PHI^{-1}) by d/dx of basis function, and add'];
            EVAL_STR(20).line = [TAB, Hess_Inv_Map_Quad_str, '.Extract_Comp_Matrix(0, Temp_MAT);'];
            EVAL_STR(21).line = [TAB, 'Scalar_Mult_Matrix(Temp_MAT, ', BF_D0, ', Temp_MAT);'];
            EVAL_STR(22).line = [TAB, 'Add_Matrix(', f_Hess_Basis_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
            
            EVAL_STR(23).line = [TAB, '// scale hessian(PHI^{-1}) by d/dy of basis function, and add'];
            EVAL_STR(24).line = [TAB, Hess_Inv_Map_Quad_str, '.Extract_Comp_Matrix(1, Temp_MAT);'];
            EVAL_STR(25).line = [TAB, 'Scalar_Mult_Matrix(Temp_MAT, ', BF_D1, ', Temp_MAT);'];
            EVAL_STR(26).line = [TAB, 'Add_Matrix(', f_Hess_Basis_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
            
            EVAL_STR(27).line = [TAB, '// scale hessian(PHI^{-1}) by d/dz of basis function, and add'];
            EVAL_STR(28).line = [TAB, Hess_Inv_Map_Quad_str, '.Extract_Comp_Matrix(2, Temp_MAT);'];
            EVAL_STR(29).line = [TAB, 'Scalar_Mult_Matrix(Temp_MAT, ', BF_D2, ', Temp_MAT);'];
            EVAL_STR(30).line = [TAB, 'Add_Matrix(', f_Hess_Basis_Quad_str, ', Temp_MAT, ', f_Hess_Basis_Quad_str, ');'];
        end
    else
        error('Invalid!');
    end

...
#define SpecificFUNC        Data_Type_U_Space_phi_restricted_to_Gamma
#define SpecificFUNC_str   "Data_Type_U_Space_phi_restricted_to_Gamma"
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            {
            const VEC_2x1  grad_basis_vec(phi_0_Basis_Val_1_0_0[qp_i][basis_i], phi_0_Basis_Val_0_1_0[qp_i][basis_i]);
            Mat_Transpose_Vec(Mesh->Map_PHI_Inv_Grad[0], grad_basis_vec, Func_f_Grad[basis_i][qp_i]);
            }
        }
/*------------   END: Auto Generate ------------*/
}

...
// FEM matrix is:
//
// \int_{Gamma}  lambda_v1_t1*v_v1_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Bdy_Mass
#define SpecificFEM_str   "Bdy_Mass"
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    ...
    const int row_elem_index = U_Space_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    U_Space_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = M_Space_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    M_Space_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*M_Space_phi_restricted_to_Gamma->Func_f_Value)[j][qp].a*(*U_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;

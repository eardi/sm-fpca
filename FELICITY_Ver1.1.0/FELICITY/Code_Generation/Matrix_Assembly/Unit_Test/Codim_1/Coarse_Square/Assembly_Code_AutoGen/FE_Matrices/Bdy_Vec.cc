...
// FEM matrix is:
//
// \int_{Gamma}  mu_v1_t1_ds*old_soln_v1_t1_grad1
//
/*------------   END: Auto Generate ------------*/
                           ...
                           const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            {
            double  integrand = M_Space_phi_restricted_to_Gamma->Func_f_d_ds[i][qp].a*old_soln_restricted_to_Gamma->Func_f_Grad[0][qp].v[0];

...
// FEM matrix is:
//
// \int_{Gamma}  lambda_v1_t1*mu_v1_t1
//
/*------------   END: Auto Generate ------------*/
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*M_Space_phi_restricted_to_Gamma->Func_f_Value)[j][qp].a*(*M_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;

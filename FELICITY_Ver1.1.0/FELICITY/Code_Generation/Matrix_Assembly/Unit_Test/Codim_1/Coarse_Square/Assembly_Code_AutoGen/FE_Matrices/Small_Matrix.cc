...
// FEM matrix is:
//
// \int_{Gamma}  geomGamma_X1,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Small_Matrix
...
private:
    ...
    void Tabulate_Tensor_1(double*, const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma&);
    void Tabulate_Tensor_2(double*, const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma&);
    void Tabulate_Tensor_3(double*, const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma&);
    /*------------   END: Auto Generate ------------*/
};
...
FEM_MATRIX_Class () // call the base class constructor
{
    ...
    SubMAT_Info[1].Shift_Row_Index = 0*row_basis_func->Num_Nodes;
    SubMAT_Info[1].Shift_Col_Index = 1*col_basis_func->Num_Nodes;
    SubMAT_Info[2].Shift_Row_Index = 1*row_basis_func->Num_Nodes;
    SubMAT_Info[2].Shift_Col_Index = 0*col_basis_func->Num_Nodes;
    SubMAT_Info[3].Shift_Row_Index = 1*row_basis_func->Num_Nodes;
    SubMAT_Info[3].Shift_Col_Index = 1*col_basis_func->Num_Nodes;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    ...
    Tabulate_Tensor_3(SubMAT_Info[3].Local_Mat_Data, Mesh);
    MAT->add_entries(Row_Indices,     Col_Indices,    SubMAT_Info[3].Local_Mat_Data,
                     ROW_NB,          COL_NB,
                     SubMAT_Info[3].Shift_Row_Index,  SubMAT_Info[3].Shift_Col_Index);
                           ...
                           const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        double  integrand = sin(geom_Gamma_embedded_in_Omega_restricted_to_Gamma->Map_PHI[qp].v[0]);
        A[0] += integrand * Mesh.Map_Det_Jac_w_Weight[qp].a;
        }
}
/***************************************************************************************/
...
/***************************************************************************************/
/* Tabulate the tensor for the local element contribution */
void SpecificFEM::Tabulate_Tensor_3(double* A, 
                           const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    const unsigned int NQ = 5;

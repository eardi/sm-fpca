...
// FEM matrix is:
//
// \int_{Gamma}  lambda_v1_t1*theta_v1_t1
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    ...
    const int row_elem_index = E_Space_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    E_Space_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);

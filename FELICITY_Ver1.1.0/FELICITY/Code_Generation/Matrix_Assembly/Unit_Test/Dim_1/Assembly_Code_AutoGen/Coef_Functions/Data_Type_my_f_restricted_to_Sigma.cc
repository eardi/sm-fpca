...
#define SpecificNAME       "my_f"
#define SpecificFUNC        Data_Type_my_f_restricted_to_Sigma
#define SpecificFUNC_str   "Data_Type_my_f_restricted_to_Sigma"
...
public:
    ...
    const Data_Type_Vector_P1_phi_restricted_to_Sigma*     basis_func;
    ...
    // arc-length derivative of coefficient function
    SCALAR Func_f_d_ds[NC][NQ];

    // constructor
    SpecificFUNC ();
    ~SpecificFUNC (); // destructor
    void Setup_Function_Space(const mxArray*, const Data_Type_Vector_P1_phi_restricted_to_Sigma*);
    void Compute_Func();
private:
};
/*------------   END: Auto Generate ------------*/
...
SpecificFUNC::SpecificFUNC ()
{
    ...
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Func_f_d_ds[nc_i][qp_i].Set_To_Zero();
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFUNC::Setup_Function_Space(const mxArray* Values,        // inputs
                                        const Data_Type_Vector_P1_phi_restricted_to_Sigma*   basis_func_input)          // inputs
{
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    // get arc-length derivative of coefficient function
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            {
            Func_f_d_ds[nc_i][qp_i].a = basis_func->Func_f_d_ds[0][qp_i].a * Node_Value[nc_i][kc[0]]; // first basis function
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                Func_f_d_ds[nc_i][qp_i].a += basis_func->Func_f_d_ds[basis_i][qp_i].a * Node_Value[nc_i][kc[basis_i]];
                }
            }
        }
/*------------   END: Auto Generate ------------*/

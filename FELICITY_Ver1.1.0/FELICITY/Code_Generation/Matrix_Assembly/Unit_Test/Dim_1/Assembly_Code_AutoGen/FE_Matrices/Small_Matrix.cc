...
// FEM matrix is:
//
// \int_{Sigma}  (old_soln_v1_t1 - exp(geomSigma_X1))^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/
                           ...
                           const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        double  integrand = old_soln_restricted_to_Sigma->Func_f_d_ds[0][qp].a;
        A[0] += integrand * Mesh.Map_Det_Jac_w_Weight[qp].a;
        }
}
/***************************************************************************************/

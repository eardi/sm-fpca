...
// FEM matrix is:
//
// \int_{Sigma}  u_v1_t1_ds*v_v1_t1_ds
//
/*------------   END: Auto Generate ------------*/
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = j; i < ROW_NB; i++)
                {
                double  integrand = Scalar_P2_phi_restricted_to_Sigma->Func_f_d_ds[j][qp].a*Scalar_P2_phi_restricted_to_Sigma->Func_f_d_ds[i][qp].a;

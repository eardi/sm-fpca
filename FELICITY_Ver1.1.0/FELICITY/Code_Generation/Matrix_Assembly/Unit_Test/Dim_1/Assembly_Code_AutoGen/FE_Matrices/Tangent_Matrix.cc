...
// FEM matrix is:
//
// \int_{Sigma}  geomSigma_T1*vv_v1_t1 + geomSigma_T2*vv_v1_t2 + geomSigma_T3*vv_v1_t3
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Tangent_Matrix
#define SpecificFEM_str   "Tangent_Matrix"
...
public:
    ...
    const Data_Type_my_f_restricted_to_Sigma*  my_f_restricted_to_Sigma;
    const Data_Type_old_soln_restricted_to_Sigma*  old_soln_restricted_to_Sigma;
    /*------------   END: Auto Generate ------------*/
    ...
    void Add_Entries_To_Global_Matrix (const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma&);
    /*------------   END: Auto Generate ------------*/
private:
    /*------------ BEGIN: Auto Generate ------------*/
    void Tabulate_Tensor_0(double*, const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma&);
    void Tabulate_Tensor_1(double*, const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma&);
    void Tabulate_Tensor_2(double*, const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma&);
    /*------------   END: Auto Generate ------------*/
};
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    // get local to global index map for the current ROW element
    const int row_elem_index = Vector_P1_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Vector_P1_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
                           ...
                           const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            {
            double  integrand = geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Tangent_Vector[qp].v[0]*(*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;
                           ...
                           const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            {
            double  integrand = geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Tangent_Vector[qp].v[2]*(*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;
            A[i] += integrand * Mesh.Map_Det_Jac_w_Weight[qp].a;
            }
        }
}

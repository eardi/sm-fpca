...
// FEM matrix is:
//
// \int_{Sigma}  uu_v1_t1*vv_v1_t1 + uu_v1_t2*vv_v1_t2 + uu_v1_t3*vv_v1_t3
//
/*------------   END: Auto Generate ------------*/
...
// the col function space is Type = CG, Name = "lagrange_deg1_dim1"
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    const int col_elem_index = Vector_P1_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Vector_P1_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[j][qp].a*(*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;

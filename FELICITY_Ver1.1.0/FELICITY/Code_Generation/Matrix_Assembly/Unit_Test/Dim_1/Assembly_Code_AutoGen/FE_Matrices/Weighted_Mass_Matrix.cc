...
// FEM matrix is:
//
// \int_{Sigma}  my_f_v1_t1_ds*u_v1_t1*v_v1_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Weighted_Mass_Matrix
#define SpecificFEM_str   "Weighted_Mass_Matrix"
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    const int row_elem_index = Scalar_P2_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Scalar_P2_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = Scalar_P2_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Scalar_P2_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = my_f_restricted_to_Sigma->Func_f_d_ds[0][qp].a*(*Scalar_P2_phi_restricted_to_Sigma->Func_f_Value)[j][qp].a*(*Scalar_P2_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;

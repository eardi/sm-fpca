...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    const int row_elem_index = Scalar_P2_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Scalar_P2_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = Scalar_P2_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Scalar_P2_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*Scalar_P2_phi_restricted_to_Gamma->Func_f_Value)[j][qp].a*(*Scalar_P2_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;

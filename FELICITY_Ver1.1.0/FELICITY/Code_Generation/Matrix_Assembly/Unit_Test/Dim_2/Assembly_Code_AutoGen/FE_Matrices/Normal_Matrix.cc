...
// FEM matrix is:
//
// \int_{Gamma}  geomGamma_N1*vv_v1_t1 + geomGamma_N2*vv_v1_t2 + geomGamma_N3*vv_v1_t3
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Normal_Matrix
#define SpecificFEM_str   "Normal_Matrix"

// the row function space is Type = CG, Name = "lagrange_deg1_dim2"
...
public:
    ...
    const Data_Type_Scalar_P2_phi_restricted_to_Gamma*  Scalar_P2_phi_restricted_to_Gamma;
    const Data_Type_Vector_P1_phi_restricted_to_Gamma*  Vector_P1_phi_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/
    ...
    void Add_Entries_To_Global_Matrix (const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma&);
    /*------------   END: Auto Generate ------------*/
private:
    /*------------ BEGIN: Auto Generate ------------*/
    void Tabulate_Tensor_0(double*, const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma&);
    void Tabulate_Tensor_1(double*, const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma&);
    void Tabulate_Tensor_2(double*, const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma&);
    /*------------   END: Auto Generate ------------*/
};
...
FEM_MATRIX_Class () // call the base class constructor
{
    ...
    SubMAT_Info[1].Shift_Row_Index = 1*row_basis_func->Num_Nodes;
    SubMAT_Info[1].Shift_Col_Index = 0*col_basis_func->Num_Nodes;
    SubMAT_Info[2].Shift_Row_Index = 2*row_basis_func->Num_Nodes;
    SubMAT_Info[2].Shift_Col_Index = 0*col_basis_func->Num_Nodes;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    // get local to global index map for the current ROW element
    const int row_elem_index = Vector_P1_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Vector_P1_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    ...
    Tabulate_Tensor_1(SubMAT_Info[1].Local_Mat_Data, Mesh);
    MAT->add_entries(Row_Indices,     Col_Indices,    SubMAT_Info[1].Local_Mat_Data,
                     ROW_NB,          COL_NB,
                     SubMAT_Info[1].Shift_Row_Index,  SubMAT_Info[1].Shift_Col_Index);

    Tabulate_Tensor_2(SubMAT_Info[2].Local_Mat_Data, Mesh);
    MAT->add_entries(Row_Indices,     Col_Indices,    SubMAT_Info[2].Local_Mat_Data,
                     ROW_NB,          COL_NB,
                     SubMAT_Info[2].Shift_Row_Index,  SubMAT_Info[2].Shift_Col_Index);
                           ...
                           const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            {
            double  integrand = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[0]*(*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;
...
/***************************************************************************************/
/* Tabulate the tensor for the local element contribution */
void SpecificFEM::Tabulate_Tensor_1(double* A, 
                           const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    const unsigned int NQ = 19;
...
/***************************************************************************************/
/* Tabulate the tensor for the local element contribution */
void SpecificFEM::Tabulate_Tensor_2(double* A, 
                           const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    const unsigned int NQ = 19;
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            {
            double  integrand = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[2]*(*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;
            A[i] += integrand * Mesh.Map_Det_Jac_w_Weight[qp].a;
            }
        }
}

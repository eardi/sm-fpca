...
// FEM matrix is:
//
// \int_{Gamma}  uu_v1_t1*vv_v1_t1 + uu_v1_t2*vv_v1_t2 + uu_v1_t3*vv_v1_t3
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Vector_Mass_Matrix
#define SpecificFEM_str   "Vector_Mass_Matrix"
...
// the col function space is Type = CG, Name = "lagrange_deg1_dim2"
...
FEM_MATRIX_Class () // call the base class constructor
{
    ...
    SubMAT_Info[1].Shift_Col_Index = 1*col_basis_func->Num_Nodes;
    SubMAT_Info[2].Shift_Row_Index = 2*row_basis_func->Num_Nodes;
    SubMAT_Info[2].Shift_Col_Index = 2*col_basis_func->Num_Nodes;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    const int col_elem_index = Vector_P1_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Vector_P1_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[j][qp].a*(*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;

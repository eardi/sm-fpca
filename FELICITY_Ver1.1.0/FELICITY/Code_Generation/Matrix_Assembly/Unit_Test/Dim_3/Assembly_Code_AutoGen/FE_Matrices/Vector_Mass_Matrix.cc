...
// FEM matrix is:
//
// \int_{Omega}  uu_v1_t1*vv_v1_t1 + uu_v1_t2*vv_v1_t2 + uu_v1_t3*vv_v1_t3
//
/*------------   END: Auto Generate ------------*/
...
private:
    ...
    void Tabulate_Tensor_1(double*, const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega&);
    void Tabulate_Tensor_2(double*, const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega&);
    /*------------   END: Auto Generate ------------*/
};
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    const int row_elem_index = Vector_P1_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Vector_P1_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = Vector_P1_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Vector_P1_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*Vector_P1_phi_restricted_to_Omega->Func_f_Value)[j][qp].a*(*Vector_P1_phi_restricted_to_Omega->Func_f_Value)[i][qp].a;

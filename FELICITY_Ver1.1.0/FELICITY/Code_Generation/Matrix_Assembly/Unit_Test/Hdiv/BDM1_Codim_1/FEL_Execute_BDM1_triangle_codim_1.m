...
Omega_Tri = uint32(Omega_Mesh.Triangulation);
...
F1 = @(X) [1 + 0*X(:,1), 0*X(:,2)];
ff_VEC = Get_BDM1_Interpolant_of_Function(Omega_Mesh.X,Omega_Tri,BDM1_DoFmap,Box_Orient(Omega_Tri_Indices,:),F1);
...
% assemble
tic
FEM = feval(str2func(mexName_assembly),[],MT.X,uint32(MT.Triangulation),Box_Orient,Subdomain_Embed,BDM1_DoFmap);
toc

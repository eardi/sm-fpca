...
#define SpecificNAME       "old_vel"
#define SpecificFUNC        Data_Type_old_vel_restricted_to_Omega
#define SpecificFUNC_str   "Data_Type_old_vel_restricted_to_Omega"
...
public:
    ...
    const Data_Type_RT0_phi_restricted_to_Omega*     basis_func;
    ...
    // divergence of coefficient function evaluated at a quadrature point in reference element
    SCALAR Func_vv_Div[NC][NQ];

    // constructor
    SpecificFUNC ();
    ~SpecificFUNC (); // destructor
    void Setup_Function_Space(const mxArray*, const Data_Type_RT0_phi_restricted_to_Omega*);
    void Compute_Func();
private:
};
/*------------   END: Auto Generate ------------*/
...
SpecificFUNC::SpecificFUNC ()
{
    ...
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Func_vv_Div[nc_i][qp_i].Set_To_Zero();
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFUNC::Setup_Function_Space(const mxArray* Values,        // inputs
                                        const Data_Type_RT0_phi_restricted_to_Omega*   basis_func_input)          // inputs
{
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    // get coefficient (divergence) function values
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            {
            Func_vv_Div[nc_i][qp_i].a = Node_Value[nc_i][kc[0]] * basis_func->Func_vv_Div[0][qp_i].a;
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                Func_vv_Div[nc_i][qp_i].a += Node_Value[nc_i][kc[basis_i]] * basis_func->Func_vv_Div[basis_i][qp_i].a;
                }
            }
        }
/*------------   END: Auto Generate ------------*/

...
// FEM matrix is:
//
// \int_{Omega}  p_v1_t1*vv_t1_div
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Div_Matrix
#define SpecificFEM_str   "Div_Matrix"
...
// the col function space is Type = DG, Name = "lagrange_deg0_dim2"
...
public:
    ...
    const Data_Type_RT0_phi_restricted_to_Omega*  RT0_phi_restricted_to_Omega;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    const int row_elem_index = RT0_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    RT0_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = P0_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    P0_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);

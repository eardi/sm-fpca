...
// FEM matrix is:
//
// \int_{Omega}  uu_v1_t1*vv_v1_t1 + uu_v2_t1*vv_v2_t1
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    const int col_elem_index = RT0_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    RT0_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);

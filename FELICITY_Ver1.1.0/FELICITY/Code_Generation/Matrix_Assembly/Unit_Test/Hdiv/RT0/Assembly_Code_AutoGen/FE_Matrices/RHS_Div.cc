...
// FEM matrix is:
//
// \int_{Omega}  (2778046668940015*q_v1_t1*sin(geomOmega_X1*pi)*sin(geomOmega_X2*pi))/140737488355328
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        RHS_Div
#define SpecificFEM_str   "RHS_Div"

// the row function space is Type = DG, Name = "lagrange_deg0_dim2"
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    const int row_elem_index = P0_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    P0_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);

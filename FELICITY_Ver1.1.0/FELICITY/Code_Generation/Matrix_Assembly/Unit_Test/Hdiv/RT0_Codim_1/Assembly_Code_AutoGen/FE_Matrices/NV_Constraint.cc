...
// FEM matrix is:
//
// \int_{Boundary}  q_v1_t1*(geomBoundary_N1*uu_v1_t1 + geomBoundary_N2*uu_v2_t1)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        NV_Constraint
#define SpecificFEM_str   "NV_Constraint"

// the row function space is Type = DG, Name = "lagrange_deg0_dim1"
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Boundary_embedded_in_Omega_restricted_to_Boundary& Mesh)
{
    ...
    const int row_elem_index = P0_phi_restricted_to_Boundary->Mesh->Domain->Sub_Cell_Index;
    P0_phi_restricted_to_Boundary->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);

...
// FEM matrix is:
//
// \int_{Boundary}  (geomBoundary_N1*uu_v1_t1 + geomBoundary_N2*uu_v2_t1)*(geomBoundary_N1*vv_v1_t1 + geomBoundary_N2*vv_v2_t1)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        NV_Matrix
#define SpecificFEM_str   "NV_Matrix"

// the row function space is Type = CG, Name = "raviart_thomas_deg0_dim2"
...
// the col function space is Type = CG, Name = "raviart_thomas_deg0_dim2"
...
public:
    ...
    const CLASS_geom_Boundary_embedded_in_Omega_restricted_to_Boundary*  geom_Boundary_embedded_in_Omega_restricted_to_Boundary;
    const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Boundary*  geom_Omega_embedded_in_Omega_restricted_to_Boundary;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pointers for accessing FE basis functions
    const Data_Type_P0_phi_restricted_to_Boundary*  P0_phi_restricted_to_Boundary;
    const Data_Type_RT0_phi_restricted_to_Boundary*  RT0_phi_restricted_to_Boundary;
    /*------------   END: Auto Generate ------------*/
    ...
    void Add_Entries_To_Global_Matrix (const CLASS_geom_Boundary_embedded_in_Omega_restricted_to_Boundary&);
    /*------------   END: Auto Generate ------------*/
private:
    /*------------ BEGIN: Auto Generate ------------*/
    void Tabulate_Tensor_0(double*, const CLASS_geom_Boundary_embedded_in_Omega_restricted_to_Boundary&);
    /*------------   END: Auto Generate ------------*/
};
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Boundary_embedded_in_Omega_restricted_to_Boundary& Mesh)
{
    // get local to global index map for the current ROW element
    const int row_elem_index = RT0_phi_restricted_to_Boundary->Mesh->Domain->Sub_Cell_Index;
    RT0_phi_restricted_to_Boundary->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = RT0_phi_restricted_to_Boundary->Mesh->Domain->Sub_Cell_Index;
    RT0_phi_restricted_to_Boundary->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
...
void SpecificFEM::Tabulate_Tensor_0(double* A, 
                           const CLASS_geom_Boundary_embedded_in_Omega_restricted_to_Boundary& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (geom_Boundary_embedded_in_Omega_restricted_to_Boundary->Map_Normal_Vector[0].v[0]*RT0_phi_restricted_to_Boundary->Func_vv_Value[j][qp].v[0]+geom_Boundary_embedded_in_Omega_restricted_to_Boundary->Map_Normal_Vector[0].v[1]*RT0_phi_restricted_to_Boundary->Func_vv_Value[j][qp].v[1])*(geom_Boundary_embedded_in_Omega_restricted_to_Boundary->Map_Normal_Vector[0].v[0]*RT0_phi_restricted_to_Boundary->Func_vv_Value[i][qp].v[0]+geom_Boundary_embedded_in_Omega_restricted_to_Boundary->Map_Normal_Vector[0].v[1]*RT0_phi_restricted_to_Boundary->Func_vv_Value[i][qp].v[1]);

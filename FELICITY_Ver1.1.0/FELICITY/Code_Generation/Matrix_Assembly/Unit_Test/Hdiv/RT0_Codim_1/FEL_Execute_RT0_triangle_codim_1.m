...
F1 = @(X) [X(2); X(1)];
uu_VEC = Get_RT0_Interpolant_of_Function(MT.X,MT.Triangulation,RT0_DoFmap,Omega_Orient,F1);
% store the coefficient function values
F2 = @(X) [X(2)^2; exp(-X(1))];
old_vel = Get_RT0_Interpolant_of_Function(MT.X,MT.Triangulation,RT0_DoFmap,Omega_Orient,F2);
Boundary_Edges = MT.Get_Global_Subdomain('Boundary');
Edge_Mid_Pt = 0.5 * (MT.X(Boundary_Edges(:,1),:) + MT.X(Boundary_Edges(:,2),:));
...
% assemble
tic
FEM = feval(str2func(mexName),[],MT.X,uint32(MT.Triangulation),Omega_Orient,Subdomain_Embed,P0_DoFmap,RT0_DoFmap);
toc

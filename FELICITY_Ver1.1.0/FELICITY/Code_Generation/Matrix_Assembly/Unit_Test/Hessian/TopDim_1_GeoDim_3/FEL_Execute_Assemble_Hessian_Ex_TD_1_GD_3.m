function status = FEL_Execute_Assemble_Hessian_Ex_TD_1_GD_3(mexName)
%FEL_Execute_Assemble_Hessian_Ex_TD_1_GD_3
%
...
Num_P1_Nodes = 81; % best relative accuracy for 4th order stiffness matrix is ~1e-6
...
Ref_P2_Elem = ReferenceFiniteElement(lagrange_deg2_dim1,1);
...
Mesh = Mesh.Set_X(P1_Mesh_Nodes);

% compute the arc-length at each mesh node
%arc_length_func = @(t) asinh(2*t)/4 + t.*(t.^2 + 1/4).^(1/2);
arc_length_func = @(t) (2^(1/2)*asinh(2*2^(1/2)*t))/8 + 2^(1/2)*t.*(t.^2 + 1/8).^(1/2);
...
Vector_Values = [(exp(arc_length_s)-1), exp(-arc_length_s), 0*arc_length_s];
...
toc
RefFEMDataFileName = fullfile(Current_Dir,'FEL_Execute_Assemble_Hessian_Ex_TD_1_GD_3_REF_Data.mat');
% FEM_REF = FEM;
...
% Func = @(t) 8 * (1+8*t.^2).^(-5/2) .* (exp(2*arc_length_func(t)) + exp(-2*arc_length_func(t)));
% QQ = integral(Func,0,1,'AbsTol',1e-14);
% %QQ = 4.567969860496733

V1 = ( (exp(2*Length_of_Sigma) - exp(-2*Length_of_Sigma)) / 2 ) + 4.567969860496733;

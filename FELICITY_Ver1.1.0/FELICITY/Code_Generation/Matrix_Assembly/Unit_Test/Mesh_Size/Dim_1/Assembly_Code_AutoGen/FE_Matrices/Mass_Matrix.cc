...
// the col function space is Type = DG, Name = "lagrange_deg0_dim1"
...
public:
    ...
    const Data_Type_Scalar_P0_phi_restricted_to_Sigma*  Scalar_P0_phi_restricted_to_Sigma;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix(const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    const int row_elem_index = Scalar_P0_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Scalar_P0_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices);
    // get local to global index map for the current COL element
    const int col_elem_index = Scalar_P0_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Scalar_P0_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices);
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        for (unsigned int j = 0; j < COL_NB; j++)
            ...
            for (unsigned int i = 0; i < ROW_NB; i++)
                {
                double  integrand = (*Scalar_P0_phi_restricted_to_Sigma->Func_f_Value)[j][qp].a*(*Scalar_P0_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;

...
M_Space_DoFmap = uint32(Omega_1_Mesh.Triangulation);
%M_Space_DoFmap

my_lambda = sin(pi*Omega_1_Mesh.X(:,1)); % sin(pi*x)
my_u      = Omega_Mesh.X(:,1).^3; % x^3
...
figure;
TEMP = [Omega_Mesh.X, (1:1:length(Omega_Mesh.X))'];
SX = sortrows(TEMP,1);
PV = SX(:,2);
h1 = plot(Omega_Mesh.X(PV,1),Soln(PV,1),'b-','LineWidth',2.0);

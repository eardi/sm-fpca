...
V1_Space_DoFmap = uint32(Omega_1_Mesh.Triangulation);
%V1_Space_DoFmap

Omega_2_Mesh = Mesh.Output_Subdomain_Mesh('Omega_2');
V2_Space_DoFmap = uint32(Omega_2_Mesh.Triangulation);
%V2_Space_DoFmap
...
old_soln_Values = Omega_Mesh.X(:,1); % the function x
my_mu     = Gamma_Mesh.X(:,2).^3; % y^3
my_lambda = sin(pi*Gamma_Mesh.X(:,2));   % sin(pi*y)
my_v1     = sin(pi*Omega_1_Mesh.X(:,2)); % sin(pi*y)
my_v2     = sin(pi*Omega_2_Mesh.X(:,2)); % sin(pi*y)
my_v      = Omega_Mesh.X(:,1).*Omega_Mesh.X(:,2); % x*y

% assemble
tic
FEM = feval(str2func(mexName),[],Mesh.X,uint32(Mesh.Triangulation),[],Subdomain_Embed,M_Space_DoFmap,U_Space_DoFmap,...
...
other_v = sin(pi*Omega_Mesh.X(:,2)); % sin(pi*y)

...
U_Space_DoFmap = uint32(Omega_Mesh.Triangulation);
%U_Space_DoFmap

Gamma_Mesh = Mesh.Output_Subdomain_Mesh('Gamma');
M_Space_DoFmap = uint32(Gamma_Mesh.Triangulation);
%M_Space_DoFmap

Sigma_Mesh = Mesh.Output_Subdomain_Mesh('Sigma');
E_Space_DoFmap = uint32(Sigma_Mesh.Triangulation);
%E_Space_DoFmap

my_v  = Omega_Mesh.X(:,1) + Omega_Mesh.X(:,2) + Omega_Mesh.X(:,3);   % x + y + z
my_u  = 2*Omega_Mesh.X(:,3) - Omega_Mesh.X(:,1) + Omega_Mesh.X(:,2); % 2z - x + y
my_p  = (1/sqrt(2))*(Gamma_Mesh.X(:,1) + Gamma_Mesh.X(:,2)) - Gamma_Mesh.X(:,3); % (1/sqrt(2)) * (x + y) - z
my_q1 = Gamma_Mesh.X(:,1) - Gamma_Mesh.X(:,2); % x - y
my_q2 = Gamma_Mesh.X(:,3); % z
my_lam = Sigma_Mesh.X(:,1).*Sigma_Mesh.X(:,2).*Sigma_Mesh.X(:,3); % x*y*z
old_soln_values = Omega_Mesh.X(:,1).*Omega_Mesh.X(:,2).*Omega_Mesh.X(:,3); % the function x*y*z

% assemble
tic
FEM = feval(str2func(mexName),[],Mesh.X,uint32(Mesh.Triangulation),[],Subdomain_Embed,...

...
% assemble
tic
FEM = DEMO_mex_Heat_Eqn_On_A_Surface([],Mesh.X,uint32(Mesh.Triangulation),[],[],Vh_DoFmap);
toc
Mass  = FEM(1).MAT;
Stiff = FEM(2).MAT;
...
x = Mesh.X(:,1); y = Mesh.X(:,2); z = Mesh.X(:,3);
%min_x = min(x);
...
for ind = 1:6
    ...
    h1 = trisurf(Mesh.Triangulation,Mesh.X(:,1),Mesh.X(:,2),Mesh.X(:,3),Soln(:,time_index));
    shading interp;
    colormap(jet);
    %light;
    lightangle(80,-20);

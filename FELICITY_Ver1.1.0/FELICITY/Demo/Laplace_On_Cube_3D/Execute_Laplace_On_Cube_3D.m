...
P1_Space_DoFmap = uint32(Mesh.Triangulation);

% assemble
disp(' ');
disp('Assemble Stiffness matrix:');
tic
FEM = DEMO_mex_Laplace_On_Cube_3D_assemble([],Mesh.X,P1_Space_DoFmap,[],[],P1_Space_DoFmap);
toc
...
disp('Assemble AGAIN!:');
tic
FEM = DEMO_mex_Laplace_On_Cube_3D_assemble(FEM,Mesh.X,P1_Space_DoFmap,[],[],P1_Space_DoFmap);
toc
...
figure;
h1 = trisurf(Bdy_Faces,Mesh.X(:,1),Mesh.X(:,2),Mesh.X(:,3),Soln(:,1));

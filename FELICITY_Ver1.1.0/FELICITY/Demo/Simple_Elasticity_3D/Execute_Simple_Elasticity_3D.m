...
FF_center = (1/3) * (Mesh.X(Bdy_Faces(:,1),:) + Mesh.X(Bdy_Faces(:,2),:) + Mesh.X(Bdy_Faces(:,3),:));
% find faces on top
...
Col_Edge_Vtx_Mask = (Mesh.X(:,1) < 0 + 1e-5) & (Mesh.X(:,2) < 0 + 1e-5);
Col_Edge_Vtx = Mesh.X(Col_Edge_Vtx_Mask,:);
...
P1_DoFmap = uint32(Mesh.Triangulation);
...
% assemble
tic
FEM = DEMO_mex_Simple_Elasticity_3D_assemble([],Mesh.X,uint32(Mesh.Triangulation),[],Subdomain_Embed,P1_DoFmap,Displace);
toc
...
Recalc_FEM = DEMO_mex_Simple_Elasticity_3D_assemble([],Mesh.X,uint32(Mesh.Triangulation),[],Subdomain_Embed,P1_DoFmap,Displace);
...
subplot(1,2,2);
New_Mesh = Mesh.Set_X(Mesh.X + Displace); % make a new displaced mesh
h2 = New_Mesh.Plot;

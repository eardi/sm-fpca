...
EE_center = (1/2) * (Mesh.X(Bdy_Edges(:,1),:) + Mesh.X(Bdy_Edges(:,2),:));
% find edges on top
...
% allocate DoFs
Pressure_DoFmap = uint32(Mesh.Triangulation);
tic
Lag_P2_DoFmap = DEMO_mex_DoF_Lagrange_P2_Allocator_2D(uint32(Mesh.Triangulation));
...
% assemble
tic
FEM = DEMO_mex_Stokes_2D_assemble([],Mesh.X,uint32(Mesh.Triangulation),[],Subdomain_Embed,...
...
subplot(2,2,3);
h3 = trisurf(Mesh.Triangulation,Mesh.X(:,1),Mesh.X(:,2),Pressure_Soln);
shading interp;

function obj = Get_Cut_Info(obj,LS)
%Get_Cut_Info
%
...
%
%    obj = obj.Get_Cut_Info(LS);
%
%    LS  = level set object.

% Copyright (c) 11-04-2011,  Shawn W. Walker

T0 = clock;
% evaluate the level set function at all vertices
obj.LS.value = LS.Interpolate(obj.bcc_mesh.V);
...
obj.cut_info.short.points = obj.Compute_Cut_Points(LS, obj.bcc_mesh.V(se(:,1),:), obj.bcc_mesh.V(se(:,2),:));
le = obj.bcc_mesh.edge.long(obj.cut_info.long.edge_indices,:);
obj.cut_info.long.points  = obj.Compute_Cut_Points(LS, obj.bcc_mesh.V(le(:,1),:), obj.bcc_mesh.V(le(:,2),:));
%%%%%%
T1 = clock;

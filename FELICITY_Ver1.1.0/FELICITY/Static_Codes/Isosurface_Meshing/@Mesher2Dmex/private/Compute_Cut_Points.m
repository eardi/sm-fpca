function cut_pts = Compute_Cut_Points(obj,LS,Edge_Tail_Point,Edge_Head_Point)
%Compute_Cut_Points
%
%    This computes a cut point for each cut edge.

% Copyright (c) 10-12-2011,  Shawn W. Walker

TOL = obj.Tolerance;

if obj.Use_Newton
    cut_pts = get_cut_point_newton(LS,Edge_Tail_Point,Edge_Head_Point,TOL);
else
    cut_pts = get_cut_point_bisection(LS,Edge_Tail_Point,Edge_Head_Point,TOL);
end

end

function pt = get_cut_point_bisection(LS,X0,X1,TOL)

t_win = [zeros(size(X0,1),1), ones(size(X0,1),1)]; % [a, b]
f_a = LS.Interpolate(X0); % f(a)
f_b = LS.Interpolate(X1); % f(b)
...
function pt = get_cut_point_newton(LS,X0,X1,TOL)
...
while (max(abs(f)) > TOL)
    ...
    [f, grad_phi] = LS.Interpolate(TEMP_PT);

...
public:
    ...
    void Get_Vertex_Coordinates(const unsigned int& VI, double VC[2]);
    void Get_Triangle_Indices(const unsigned int& TI, unsigned int TD[3]);
    ...
    const mxArray*              mxVtx;
    const mxArray*              mxTri;
    Generic_Data<double,2>        Vtx;
    Generic_Data<double,3>        Tri;
...
void BCC::Setup_Data(const mxArray* mxBCC)      // inputs
{
    ...
    mxTri = mxGetField(mxBCC, (mwIndex) 0, "T");
    ...
    // setup triangular mesh part
    Vtx.Setup_Data("Vertex Coordinates", mxVtx, 0);
    Tri.Setup_Data("Tri Data", mxTri, 0);
...
/***************************************************************************************/
/* get Triangle indices (for a single given index)
   Note: T_ind is in the MATLAB style.
*/
void BCC::Get_Triangle_Indices(const unsigned int& T_ind,
                                 unsigned int tri_data[3])         // inputs
{
    unsigned int T_ind_C_style = T_ind-1;
    double TD[3];
    Tri.Read(T_ind_C_style, TD);
    // convert
    for (unsigned int k = 0; k < 3; k++)
        tri_data[k] = (unsigned int) TD[k];
}
/***************************************************************************************/

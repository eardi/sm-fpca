...
void GTD<myType,NUM_COLS>::Read(const int& row, myType* read_data) const // inputs
{
    ...
    if ((unsigned int) row < Orig_Num_Rows) // reading the original data
        {
        const int replace_index = Replace_Ptr[row];
        if (replace_index==-1) // original data has NOT been replaced
            {
            // read from original data
            for (unsigned int i = 0; (i < Actual_Num_Cols); i++)
                read_data[i] = Data[i][row];
            }
        else // original data has been replaced previously
            {
            // read from replacement
            for (unsigned int i = 0; (i < Actual_Num_Cols); i++)
                read_data[i] = Replace_Data[i][replace_index];
            }
        }
    else if ((unsigned int) row < Num_Rows) // reading new data
        {
        // shift index
        const unsigned int new_index = row - Orig_Num_Rows;
        // read from new
        for (unsigned int i = 0; (i < Actual_Num_Cols); i++)
                                ...
                                myType& read_data) const         // inputs
{
    ...
    if ((unsigned int) row < Orig_Num_Rows) // reading the original data
        ...
        if (replace_index==-1) // original data has NOT been replaced
            {
            // read from original data
            read_data = Data[col][row];
            }
        else // original data has been replaced previously
            {
            // read from replacement
            read_data = Replace_Data[col][replace_index];
            }
        }
    else if ((unsigned int) row < Num_Rows) // reading new data
        {
...
void GTD<myType,NUM_COLS>::Overwrite(const int& row, const myType* overwrite_data)  // inputs
{
    ...
    if ((unsigned int) row < Orig_Num_Rows) // modifying the original data
        {
        const int replace_index = Replace_Ptr[row];
        if (replace_index==-1) // first time the original data is replaced
            ...
            Replace_Ptr[row] = ((int) Replace_Data[0].size())-1; // record where it is (with C-style indexing)
            }
        else // original data has been replaced previously
            {
            // overwrite previous replacement data
            for (unsigned int i = 0; (i < Actual_Num_Cols); i++)
                Replace_Data[i][replace_index] = overwrite_data[i];
            }
        }
    else if ((unsigned int) row < Num_Rows) // modifying new data
        {
        // shift index
        const unsigned int new_index = row - Orig_Num_Rows;
...
mxArray* GTD<myType,NUM_COLS>::Copy_Final_Data()
{
    ...
    for (unsigned int ri = 0; (ri < Orig_Num_Rows); ri++)
        ...
        if (replace_index >= 0) // then that row was replaced
            ...
            for (unsigned int j = 0; (j < Actual_Num_Cols); j++)
                Output_Data_Val[ri + j*Num_Rows] = Replace_Data[j][replace_index];
            }
        }

...
public:
    ...
    void Adjust_Subdomain(const unsigned int&, const unsigned int&, const unsigned int&, const unsigned int*);
...
private:
    ...
    void Adjust_Subdomain_0D(std::multimap<unsigned int,int>::iterator, const unsigned int&, const unsigned int&,
                             const unsigned int&, const unsigned int*, std::stack<std::pair<unsigned int,int> >&);
        ...
        if (TopDim==2) // a single triangle can only appear once in a sub-domain
            ...
            if (TopDim==0)
                for (it = ret.first; it!=ret.second; it++)
                    Adjust_Subdomain_0D(it, t0, s0, e0, V, replace);
            else if (TopDim==1)
                for (it = ret.first; it!=ret.second; it++)
                    Adjust_Subdomain_1D(it, t0, s0, e0, V, replace);
            else
...
mxArray* SBD::Copy_Final_Data()
{
    ...
    if (Num_Cols==2)
        ...
        for (it = Entity_map.begin(); it != Entity_map.end(); it++)
            {
            Output_Data[0][ROW_INDEX] = (*it).first;

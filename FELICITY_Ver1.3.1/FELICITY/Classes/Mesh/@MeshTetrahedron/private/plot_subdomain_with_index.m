...
elseif (obj.Subdomain(Sub_Index).Dim==2)
    Plot_Handle = obj.Plot_Subdomain_2D(obj.Subdomain(Sub_Index));
elseif (obj.Subdomain(Sub_Index).Dim==3)
    Plot_Handle = obj.Plot_Subdomain_3D(obj.Subdomain(Sub_Index));
else
    error('Subdomain dimension is not valid!  Must be 0, 1, 2, or 3 for MeshTetrahedron.');
end

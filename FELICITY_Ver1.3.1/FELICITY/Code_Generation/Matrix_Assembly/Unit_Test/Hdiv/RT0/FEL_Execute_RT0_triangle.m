function status = FEL_Execute_RT0_triangle(mexName)
%FEL_Execute_RT0_triangle
%
...
F1 = @(X) [X(1); X(2)];
uu_VEC = Get_RT0_Interpolant_of_Function(Omega_Vtx,Omega_DoFmap,RT0_DoFmap,Omega_Orient,F1);
F2 = @(X) [exp(-X(1)); X(2)^2];
% store the coefficient function values
old_p   = zeros(Num_P0_Nodes,1);
old_vel = Get_RT0_Interpolant_of_Function(Omega_Vtx,Omega_DoFmap,RT0_DoFmap,Omega_Orient,F2);
% END: define some stuff

% assemble
tic
FEM = feval(str2func(mexName),[],Omega_Vtx,Omega_DoFmap,Omega_Orient,[],P0_DoFmap,RT0_DoFmap,old_p,old_vel);
toc
RefFEMDataFileName = fullfile(Current_Dir,'FEL_Execute_RT0_triangle_REF_Data.mat');
% FEM_REF = FEM;
% save(RefFEMDataFileName,'FEM_REF');
load(RefFEMDataFileName);

% do some checks
disp('Summing the RT0 mass matrix should be close to (240).  Error is:');
%sum(sum(FEM(3).MAT)) - (40/3)
sum(sum(FEM(3).MAT)) - 240

disp('Compute integral of (x^2 + y^2) over [0, 2] x [0, 2] with RT0 mass matrix.  Error is:');
uu_VEC' * FEM(3).MAT * uu_VEC - (32/3)

disp('Compute integral of div([x, y]) over [0, 2] x [0, 2] with RT0 div-pressure matrix.  Error is:');
...
RHS = [zeros(Num_RT0_Nodes,1); -FEM(4).MAT];
Soln = zeros(size(MAT,1),1);
All_Nodes = (1:1:size(Soln,1))';
Free_Nodes = setdiff(All_Nodes,Fixed_Pressure_Node+Num_RT0_Nodes);
...
FEM_P0 = feval(str2func(mexName),[],Omega_Vtx,Omega_DoFmap,Omega_Orient,[],P0_DoFmap,RT0_DoFmap,P0_Soln,old_vel);
...
EL = 40;
view(AZ,EL);
title('Solution of Mixed-Laplace Problem with RT_0 (P0 Solution)');

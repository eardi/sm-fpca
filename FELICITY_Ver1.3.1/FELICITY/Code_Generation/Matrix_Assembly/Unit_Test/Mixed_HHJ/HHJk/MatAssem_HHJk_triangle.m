function MATS = MatAssem_HHJk_triangle(deg_geo,deg_k,exact_u,vars)
%MatAssem_HHJk_triangle
...
[soln_func, soln_grad_func, soln_hess, f_func] = Compute_HHJk_Exact_Soln(exact_u,vars);
% % exact matrix-valued function
% rad = sqrt(gf.X(1)^2 + gf.X(2)^2);
% exact_sig = [sin(rad^2), cos(rad^2);
%              exp(-rad^2),  rad^2];
% %
% % exact_sig = [1,0;
% %              0,1];
...
soln_hess = subs(soln_hess,{vars(1), vars(2)},{gf.X(1), gf.X(2)});

RHS_L2_Proj = Linear(V_h);
RHS_L2_Proj = RHS_L2_Proj + Integral(Omega, sum(sum(tau.val .* soln_hess)) );

Small_Matrix = Real(1,1);
Small_Matrix = Small_Matrix + Integral(Omega, sum(sum(proj_hess.val)) );

L2_Proj_Error_Sq = Real(1,1);
L2_Proj_Error_Sq = L2_Proj_Error_Sq + Integral(Omega, sum(sum((proj_hess.val - soln_hess).^2)) );
...
L2_Hess_Error_Sq = L2_Hess_Error_Sq + Integral(Omega, sum(sum((discrete_hess.val - soln_hess).^2)) );

L2_Proj_Hess_Error_Sq = Real(1,1);
L2_Proj_Hess_Error_Sq = L2_Proj_Hess_Error_Sq + Integral(Omega, sum(sum((proj_hess.val - discrete_hess.val).^2)) );

...
// FE matrix is:
//
// \int_{dGamma}  (NV_h_v1_t1 - geomGamma_N1)^2 + (NV_h_v1_t2 - geomGamma_N2)^2 + (NV_h_v1_t3 - geomGamma_N3)^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_NV_Error_sq_Data_Type
#define SpecificFEM_str   "NV_Error_sq"
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_dGamma_embedded_in_Gamma_restricted_to_dGamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = pow(NV_h_restricted_to_dGamma->Func_f_Value[0][qp].a-geom_Gamma_embedded_in_Gamma_restricted_to_dGamma->Map_Normal_Vector[qp].v[0],2.0)+pow(NV_h_restricted_to_dGamma->Func_f_Value[1][qp].a-geom_Gamma_embedded_in_Gamma_restricted_to_dGamma->Map_Normal_Vector[qp].v[1],2.0)+pow(NV_h_restricted_to_dGamma->Func_f_Value[2][qp].a-geom_Gamma_embedded_in_Gamma_restricted_to_dGamma->Map_Normal_Vector[qp].v[2],2.0);

...
// FE matrix is:
//
// \int_{dGamma}  (TV_h_v1_t1 - geomdGamma_T1)^2 + (TV_h_v1_t2 - geomdGamma_T2)^2 + (TV_h_v1_t3 - geomdGamma_T3)^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_TV_Error_sq_Data_Type
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_dGamma_embedded_in_Gamma_restricted_to_dGamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = pow(TV_h_restricted_to_dGamma->Func_f_Value[0][qp].a-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_Tangent_Vector[qp].v[0],2.0)+pow(TV_h_restricted_to_dGamma->Func_f_Value[1][qp].a-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_Tangent_Vector[qp].v[1],2.0)+pow(TV_h_restricted_to_dGamma->Func_f_Value[2][qp].a-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_Tangent_Vector[qp].v[2],2.0);

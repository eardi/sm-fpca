...
// FE matrix is:
//
// \int_{dGamma}  (lambda_h_v1_t1 + (2*pi*(geomdGamma_X1*cos(2*geomdGamma_X1*pi)*cos(2*geomdGamma_X2*pi) - geomdGamma_X2*sin(2*geomdGamma_X1*pi)*sin(2*geomdGamma_X2*pi) + 2*geomdGamma_X1*geomdGamma_X2^2*cos(2*geomdGamma_X1*pi)*cos(2*geomdGamma_X2*pi) - 2*geomdGamma_X1^2*geomdGamma_X2*sin(2*geomdGamma_X1*pi)*sin(2*geomdGamma_X2*pi)))/((geomdGamma_X1^2 + geomdGamma_X2^2 + 4*geomdGamma_X1^2*geomdGamma_X2^2)^(1/2)*(geomdGamma_X1^2 + geomdGamma_X2^2 + 1)^(1/2)))^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_lambda_L2_Error_sq_Data_Type
#define SpecificFEM_str   "lambda_L2_Error_sq"
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_dGamma_embedded_in_Gamma_restricted_to_dGamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  t2 = geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0]*geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0];
        const double  t3 = geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1]*geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1];
        const double  t4 = geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0]*3.141592653589793*2.0;
        const double  t5 = geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1]*3.141592653589793*2.0;
        const double  t6 = cos(t4);
        const double  t7 = cos(t5);
        const double  t8 = sin(t4);
        const double  t9 = sin(t5);
        const double  integrand_0 = pow(lambda_h_restricted_to_dGamma->Func_f_Value[0][qp].a+3.141592653589793*1.0/sqrt(t2+t3+1.0)*1.0/sqrt(t2+t3+t2*t3*4.0)*(geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0]*t6*t7-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1]*t8*t9+geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0]*t3*t6*t7*2.0-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1]*t2*t8*t9*2.0)*2.0,2.0);

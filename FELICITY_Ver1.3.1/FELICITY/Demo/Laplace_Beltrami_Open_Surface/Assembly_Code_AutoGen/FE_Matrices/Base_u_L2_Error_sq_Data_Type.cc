...
// FE matrix is:
//
// \int_{Gamma}  (u_h_v1_t1 - cos(2*geomGamma_X2*pi)*sin(2*geomGamma_X1*pi))^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_u_L2_Error_sq_Data_Type
#define SpecificFEM_str   "u_L2_Error_sq"
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = pow(u_h_restricted_to_Gamma->Func_f_Value[0][qp].a-cos(geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[1]*3.141592653589793*2.0)*sin(geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[0]*3.141592653589793*2.0),2.0);

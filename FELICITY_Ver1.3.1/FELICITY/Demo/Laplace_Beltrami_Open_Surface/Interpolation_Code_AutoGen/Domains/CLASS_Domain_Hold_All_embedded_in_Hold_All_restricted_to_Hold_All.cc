...
#define MDC        CLASS_Domain_Hold_All_embedded_in_Hold_All_restricted_to_Hold_All
// set the name of the Global mesh (TopDim = 2)
#define GLOBAL_MESH_Name  "Hold_All"
// set the name of the subdomain (TopDim = 2)
#define SUBDOMAIN_Name  "Hold_All"
// set the name of the domain of integration (TopDim = 2)
#define DOMAIN_INTEGRATION_Name  "Hold_All"
...
void MDC::Read_Embed_Data(const unsigned int& doi_elem_index)  // inputs
{
    ...
    // error check:  make sure Global_Cell_Index is >= 0 and
    //               does not exceed the number of elements in the sub-domain
    if ( (Global_Cell_Index < 0) || ((unsigned int) Global_Cell_Index >= Num_Elem) )
        {
        mexPrintf("ERROR: The number of elements (cells) in the sub-domain is %d.\n",Num_Elem);
        mexPrintf("ERROR: But the Global_Cell_Index == %d.\n",Global_Cell_Index+1); // put into MATLAB style
        mexPrintf("ERROR: It *should* satisfy: 1 <= Global_Cell_Index <= %d.\n",Num_Elem);
        mexPrintf("\n");

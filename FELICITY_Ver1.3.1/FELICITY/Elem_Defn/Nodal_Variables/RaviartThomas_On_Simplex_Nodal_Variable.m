function Eval_Nodal_Var = RaviartThomas_On_Simplex_Nodal_Variable(Cell,Nodal_Var_Data,Indep_Vars)
%RaviartThomas_On_Simplex_Nodal_Variable
%
...
%
%   This is for the ***Raviart-Thomas*** finite element.
%
%   Format is:
...
%       {sym([(1/3); (1/3); (1/3)]), symbolic (vector) function, 'int_cell', 1, 'dof_set', k}
...
% all Raviart-Thomas nodal variables involve integrals
...
if strcmp(Nodal_Type,'int_facet') % integrate over facet
    ...
    Integrand_Func = @(vphi) (transpose(vphi) * nu) * q;
    % setup the integral evaluation
    Eval_Nodal_Var = @(vphi) integrate_facet(Integrand_Func(vphi),Xmap,Indep_Vars,local_var);

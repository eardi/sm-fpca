function Elem = lagrange_nonconform_deg1_dim2(Type_STR)
%lagrange_nonconform_deg1_dim2
%
%    This defines a reference finite element to be used by FELICITY.
%    INPUT: string = 'CG'/'DG' (continuous/DIScontinuous galerkin).
%
%    Lagrange H^1 Finite Element of degree = 1, in dimension = 2.
%
...
Elem.Basis(1).Func = {'2 * (x + y) - 1'};
Elem.Basis(2).Func = {'1 - 2 * x'};
Elem.Basis(3).Func = {'1 - 2 * y'};
...
Elem.Nodal_Var(1).Data = {[0, 1/2, 1/2], 'eval_facet', 1};
Elem.Nodal_Var(2).Data = {[1/2, 0, 1/2], 'eval_facet', 2};
Elem.Nodal_Var(3).Data = {[1/2, 1/2, 0], 'eval_facet', 3};

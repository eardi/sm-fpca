function Elem = nedelec_1stkind_deg2_dim2(Type_STR)
%nedelec_1stkind_deg2_dim2
%
%    This defines a reference finite element to be used by FELICITY.
%    INPUT: string = 'CG'/'DG' (continuous/DIScontinuous galerkin).
%
%    Nedelec (1st-kind) H(curl) Finite Element of degree = 2, in dimension = 2.
%
%
...
Elem.Basis(1).Func = {'-2*y*(4*x - 1)'; '4*x*(2*x - 1)'};
Elem.Basis(2).Func = {'-4*y*(2*y - 1)'; '2*x*(4*y - 1)'};
Elem.Basis(3).Func = {'4*y*(2*y - 1)'; '2*x + 6*y - 8*x*y - 2'};
Elem.Basis(4).Func = {'-2*y*(4*x + 4*y - 3)'; '8*x*y - 6*y - 12*x + 8*x^2 + 4'};
Elem.Basis(5).Func = {'8*x*y - 12*y - 6*x + 8*y^2 + 4'; '-2*x*(4*x + 4*y - 3)'};
Elem.Basis(6).Func = {'6*x + 2*y - 8*x*y - 2'; '4*x*(2*x - 1)'};
Elem.Basis(7).Func = {'-8*y*(x + 2*y - 2)'; '8*x*(x + 2*y - 1)'};
Elem.Basis(8).Func = {'8*y*(2*x + y - 1)'; '-8*x*(2*x + y - 2)'};
...
Elem.Nodal_Var(1).Data = {[0, 1, 0], '[(2^(1/2)*(y - 1))/2; -(2^(1/2)*(y - 1))/2]', 'int_edge', 1};
Elem.Nodal_Var(2).Data = {[0, 0, 1], '[-(2^(1/2)*y)/2; (2^(1/2)*y)/2]', 'int_edge', 1};
Elem.Nodal_Var(3).Data = {[0, 0, 1], '[0; y]', 'int_edge', 2};
Elem.Nodal_Var(4).Data = {[1, 0, 0], '[0; 1 - y]', 'int_edge', 2};
Elem.Nodal_Var(5).Data = {[1, 0, 0], '[1 - x; 0]', 'int_edge', 3};
Elem.Nodal_Var(6).Data = {[0, 1, 0], '[x; 0]', 'int_edge', 3};

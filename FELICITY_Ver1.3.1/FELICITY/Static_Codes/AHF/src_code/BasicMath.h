...
/* get the angles that the facets of the simplex make with respect to each other.
   inputs: global list of vertex coordinates, ordered list of vertices of the simplex.
   output: a symmetric matrix Theta giving the angles between the various facets, where
           local facet i is opposite local vertex i, and Theta(i,j) is the angle made by
           the normal vectors of facet i and facet j.  Ergo, Theta(i,i) = 0, for all i.
...
void Simplex_Angles(const BasePtCoord<GD>* const& VX, const VtxIndType* VI,
                    Eigen::Matrix<RealType, (TD+1), (TD+1)>& Theta)
{
    RealType NS_rank = 0.0; // init
    ...
    else if ( (TD==2) )
    {
        ...
        const RealType DP_01 = angle_dot_prod_normalize(TV_0.dot(TV_1));
        const RealType DP_02 = angle_dot_prod_normalize(TV_0.dot(TV_2));
        const RealType DP_12 = angle_dot_prod_normalize(TV_1.dot(TV_2));
        
        Theta(0,1) = std::acos(DP_01);
        Theta(0,2) = std::acos(DP_02);
        Theta(1,2) = std::acos(DP_12);
    ...
    else if ( (TD==3) && (GD==3) )
    {
        ...
        // get the normal vectors of each facet
        ...
        const RealType DP_01 = angle_dot_prod_normalize(NS_0.dot(NS_1));
        const RealType DP_02 = angle_dot_prod_normalize(NS_0.dot(NS_2));
        const RealType DP_03 = angle_dot_prod_normalize(NS_0.dot(NS_3));
        const RealType DP_12 = angle_dot_prod_normalize(NS_1.dot(NS_2));
        const RealType DP_13 = angle_dot_prod_normalize(NS_1.dot(NS_3));
        const RealType DP_23 = angle_dot_prod_normalize(NS_2.dot(NS_3));
        
        Theta(0,1) = std::acos(DP_01);
        Theta(0,2) = std::acos(DP_02);
        Theta(0,3) = std::acos(DP_03);
        Theta(1,2) = std::acos(DP_12);
        Theta(1,3) = std::acos(DP_13);
        Theta(2,3) = std::acos(DP_23);

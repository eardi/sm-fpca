...
inline void Get_Vec_Difference (const double head[2], const double tail[2], double Vec_Diff[2])
{
...
double Euclidean_Distance (const double Vec1[2], const double Vec2[2])
{
	double vec_diff[2];
	Get_Vec_Difference(Vec1, Vec2, vec_diff);
    double value = sqrt(vec_diff[0]*vec_diff[0] + vec_diff[1]*vec_diff[1]);
...
double Normalize (const double Vec_in[2], double Vec_out[2])
{
	const double LENGTH = sqrt(Vec_in[0]*Vec_in[0] + Vec_in[1]*Vec_in[1]);
 ...
 */
inline double Dot_Product (const double a[2], const double b[2])
{
	double val = a[0]*b[0] + a[1]*b[1];
	return val;
}
/***************************************************************************************/
...
//  */
// double Get_Angle (const double Va[2], const double Vb[2])
// {
